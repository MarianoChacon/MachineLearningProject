...
df=pd.DataFrame(columns=['Kernel','Gamma','C','Accuracy'])
c_value=[0.01,0.5,0.1]
c_i=[]
counter=0
for k in kernel_values:
    for i in gam:
        for j in c_value:
            svm_lin1=SVC(kernel=k,C=j,gamma=i)
            svm_lin1.fit(X_train_std,y_train)

            pred=svm_lin1.predict(X_test_std)
            print(f"{k},{i},{j},{accuracy_score(y_true=y_test,y_pred=pred,normalize=True)}")
            ker.append(k)
            accuracies1.append(accuracy_score(y_true=y_test,y_pred=pred,normalize=True))
            gam_i.append(i)
            c_i.append(j)
            df.loc[counter]=[k,i,j,accuracy_score(y_true=y_test,y_pred=pred,normalize=True)]
            counter+=1

print(df)
path="F:/Marian/Documents/Python/Proyectos mios/Social_Network_Ads/SVM_df.csv"
df.to_csv(path)
